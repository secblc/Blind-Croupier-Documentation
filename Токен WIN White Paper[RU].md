...
**Эмиссия:** Всего выпущено 500,000,000WIN. Повторной эмиссии не будет.
*Все не проданые токены, если такие имеются, сжигаются сразу же после окончания периода.*

**Period 0 цена** 1000WIN = 0.35ETH 

**Период уже начался и может закончиться в любой момент**

**Period 1 цена:** 1000WIN=0.5ETH 

**Начало: 21.09.2017; 14:00UTC.**

**Period 2-367 цена:** +0.5% от цены предыдущего периода.
**Если** в предыдущем периоде было уничтожено 20% или более WIN в новом периоде цена не повышается.
...
> Как вы наверное заметили из таблицы, в периоде 1 все предложенные токены были распределены, и стоимость токена во втором периоде повысилась на 0.5%. А в периоде 70 было сожжено 20% токенов, поэтому в соответствии с указанным в смарт-контракте алгоритме, стоимость токена в периоде 71 не повысилась. В этом и следующих разделах вы узнаете больше о предложенной экономической модели.

**5,000,000WIN:**  Предложено для Period O. Это небольшой период распределения токенов для раних инвесторов. Период закончится когда все предложены для периода токена будут распределены или вместе с началом Period 1(21.09.2017; 14:00UTC ).*Все не проданные токены, будут уничтожены в конце периода.*

**15,000,000WIN** Предложено для Period 1. **Период начнется: 21.09.2017; 14:00UTC.** Period 1 закончится когда все предложенные для периода токены будут распределены или по истечению 161 часа, после чего новый период начнется без замедлений. *Все не проданные токены, будут уничтожены в конце периода.*

 **366,000,000WIN:**  Предложено для периодов 2-367.  Для каждого из них выделено 1,000,000WIN, распределение которых пройдет в течении 23 часов(или пока все токены не будут распределены). В случае досрочной продажи, периоды идут один за другим без замедлений. В каждом новом периоде цена WIN увеличивается на 0.5% цены предыдущего периода. **Если** в предыдущем периоде было уничтожено 20% или более WIN в новом периоде цена не повышается.
