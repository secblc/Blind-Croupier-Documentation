...
Для начала смешаем сиды попарно (первым со вторым, третий с четвертым, используя исключающее ИЛИ (XOR, ^). Можно использовать, например, калькулятор: http://xor.pw/
...
Проверить финальную руку Игрока, имея оба сида и команду замены, можно вызвав функцию `generateHand` из Банка. Это можно сделать по ссылке:

https://kovan.etherscan.io/address/0xc7288d4765aee8dd536250fc1b882404377ffbb1#readContract
