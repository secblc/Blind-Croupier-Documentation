...
После перехода по ссылке появится список хэшей, транзакции партии, ставки, сиды, начальная рука, команды замены, рука после замены например:
...
Как видите честность игры действительно легко проверяется и записана в блокчейне. Мы все еще ведем работу над улучшеным интерфейсом проверки на честность.

# Самостоятельное Изучение Транзакций
Если вы хотите более глубоко погрузиться в изучение сыгрыных игр и проверить на честность игорное заведение, к сожалению самостоятельное изучение транзакций может потребовать от вас некоторых технических навыков,  поэтому мы написали небольшой но подробный гайд,

По порядку будем исследовать хэши транзакций. Для просмотра вызываемого метода и параметров вносим в ссылку необходимые данные транзакций по ссылке: `http://kovan.etherscan.io/tx/**TXHASH**`, где `**TXHASH**` - хэш транзакции из списка. Нас будет интересовать поле `Input Data`, в котором указано название функции (`submitBet`, `submitCrouiperSeedAsSignature`, `submitReplacementOrder` и т.д. ) и список ее аргументов, исследование всех методов будет происходить по схожей аналогии. 

## Шаг первый: изучение сделаных ставок
Для изучения информации о сделаных ставках необходимо подставить соответствующий ``"txHash"`` для метода `submitBet` будем следовать и дальше анализу одной конкретной партии поэтому возьмем соответствующий хэш``"0x31a7368260c0a2f8ddad9afecea9ebac1bc97a4274c695fe3ee03b3be9be93a2",`` и подставим в ссылку после чего перейдем по ней
